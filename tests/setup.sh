...
:install-sync-command-into-container() {
    ...
    tests:ensure sudo mkdir -p "$rootfs/usr/lib/cure/"
    tests:ensure sudo cp "$file_name" "$rootfs/usr/lib/cure/"
    tests:ensure sudo chmod +x "$rootfs/usr/lib/cure/sync"
}

tests:debug "!!! setup"

tests:clone "cure" "bin/"
